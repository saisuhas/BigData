...
object GraphX {
  def main(args: Array[String]): Unit = {
    ...
    val configuration = new SparkConf().setAppName("GraphX")
    ...
    //Output path
    //val outputPath = if (args.length > 1) args(1) else "GraphX"
    ...
    sc.parallelize(Seq(od + id + pr + cc + tc)).saveAsTextFile(outputPath)
